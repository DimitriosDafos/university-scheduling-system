...
@section('content')
    ...
    <div class="bg-white rounded shadow p-6 max-w-3xl mx-auto">
        <h1 class="text-2xl font-bold mb-6">{{ __('Add Event') }}</h1>
        ...
        <form action="{{ route('events.store') }}" method="POST">
            ...
            <div class="mt-4">
                ...
                <textarea name="description" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" rows="4">{{ old('description') }}</textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mt-4 flex justify-end">
                <a href="{{ route('events.index') }}" class="mr-2 px-4 py-2 border rounded">{{ __('Cancel') }}</a>
